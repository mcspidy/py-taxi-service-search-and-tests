...
from .forms import (DriverCreationForm, DriverLicenseUpdateForm, CarForm,
                    CarManufacturerSearchForm, CarModelSearchForm,
...
class ManufacturerListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["search_form"] = CarManufacturerSearchForm(
            initial={"manufacturer": manufacturer})
        return context

    def get_queryset(self):
        queryset = super().get_queryset()
        form = CarManufacturerSearchForm(self.request.GET)

        if form.is_valid():
            return queryset.filter(
                name__icontains=form.cleaned_data["manufacturer"].strip())
        return queryset
